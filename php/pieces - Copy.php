...
class Session
{
    ...
    function setWalked($x, $y, $position, $atTile, $tile, $road, &$idx)
    {
        ...
        if($this->positionExists($x, $y, $this->walked))
        {
            ...
            {
                $this->walked[$y][$x] = array($idx => $this->walkIndex);
            }
        }
        else
        {
    ...
    function walk($x, $y, $position, $road = false, $end = false)
    {
        ...
        if($atTile[2] > 0)
        {
            ...
            if($atTile[2] >> 7 == ($idx))
            {
                ...
                else if($returnOverall == $this->currentSequence)
        ...
        for($j = 1; $j < count($tilePosition); $j++)
        {
            ...
            if($return >= 0)
            {
                if($return == $this->currentSequence && !$this->walkSettings['checkClose'])
                    return $return;
    ...
    function borderMeeples(&$border, $x, $y, &$closers = NULL, $end = false)
    {
        ...
        for($i = 0; $i < 12; $i++)
        {
            ...
            if($this->walkSettings['checkClose'] && $this->closeTrue)
                $closers[$i] = array($this->closeMeeple, $this->flattenArray($this->closePieces));
